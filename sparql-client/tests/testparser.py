...
class TestParser(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        row0 = rows[0]
        self.assertEqual(sparql.IRI(u'http://rdfdata.eionet.europa.eu/eea/countries/BE'
                         ), row0[0])
        self.assertEqual(sparql.Literal('471161.0', XSD_FAO_MILLION),
                         row0[1])
        self.assertEqual(sparql.Literal('44.252934', sparql.XSD_FLOAT),
                         row0[5])
    ...
    def test_fetchmany(self):
        ...
        row0 = rows[0]
        if six.PY2:
            self.assertEqual('http://rdfdata.eionet.europa.eu/eea/countries/BE'
                             , str(row0[0]))
        else:
            self.assertEqual('http://rdfdata.eionet.europa.eu/eea/countries/BE'
                             , row0[0].value)
...
#       print row1[0]
    ...
    def test_w3_example(self):
        ...
        row0 = rows[0]
        if six.PY2:
            self.assertEqual('http://work.example.org/alice/',
                             str(row0[1]))
        else:
            self.assertEqual('http://work.example.org/alice/',
                             row0[1].value)
    ...
    def test_national(self):
        ...
        result = sparql._ResultsParser(resultfp)
        self.assertEqual([u'subj', u'nameen', u'nameru'],
                         result.variables)

        rows = result.fetchall()
        row0 = rows[0]
        if six.PY2:
            self.assertEqual('http://aims.fao.org/aos/geopolitical.owl#Germany'
                             , str(row0[0]))
        else:
            self.assertEqual('http://aims.fao.org/aos/geopolitical.owl#Germany'
                             , row0[0].value)

        self.assertEqual(sparql.IRI(u'http://aims.fao.org/aos/geopolitical.owl#Germany'
                         ), row0[0])
    ...
    def test_big_text(self):
        ...
        self.assertEqual('multiple<br>paragraphs<br>here',
                         row0[0].value)
    ...
    @patch('sparql._ResultsParser._fetchhead',
           side_effect=side_effect_fetchhead)
